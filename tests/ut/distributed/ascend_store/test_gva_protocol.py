...
class TestUseGvaLayerwise(unittest.TestCase):
    """The gate takes the caller's authoritative ``use_layerwise`` flag
    plus the raw extra config, and owns the backend config key, its
    default, and the name normalization (strip + lower).
    """

    def test_truth_table(self):
        cases = [
            (True, {"backend": "memcache"}, True),
            (False, {"backend": "memcache"}, False),
            (True, {"backend": "mooncake"}, False),
            (True, {"backend": "yuanrong"}, False),
            # The backend name is normalized (strip + lower): case/whitespace
            # variants of the memcache backend still pass the gate.
            (True, {"backend": "MEMCACHE"}, True),
            (True, {"backend": " Memcache "}, True),
            # Backend defaults to mooncake when the key is absent.
            (True, {}, False),
        ]
        for use_layerwise, extra_config, expected in cases:
            with self.subTest(use_layerwise=use_layerwise, extra_config=extra_config):
                self.assertEqual(use_gva_layerwise(use_layerwise, extra_config), expected)

    def test_unknown_backend(self):
        self.assertFalse(use_gva_layerwise(True, {"backend": "nonexistent"}))
