...
**Commit**: 785a856d
**标题**: refactor: improve log quality for better debugging
**改动统计**: 15 个文件，+401 行，-166 行
...
# 改进后
logger.warning("[cpu_binding] NPU topo affinity not found. action: fallback to global-slice CPU binding.")
```
...
**当前**: 约 50% 的 ERROR/WARNING 有排查方向
**目标**: 100% 的 ERROR/WARNING 有排查方向

**建议**: 具体问题具体分析，简短精准说明排查方向

---

### 3. 添加 trace_id

**当前**: 所有 ERROR/WARNING 都没有 trace_id
**目标**: 所有 ERROR/WARNING 都有 trace_id
...
| 方面 | PR #9537 | 我们的 Commit | 结论 |
|------|----------|--------------|------|
| 参数格式 | ⚠️ 不统一 | ✅ 统一 | 我们更好 |
| 关键参数 | ⚠️ 部分 | ✅ 完整 | 我们更好 |
| 期望值/实际值 | ❌ 无 | ✅ 有 | 我们更好 |
| 排查方向 | ❌ 几乎无 | ⚠️ 约 50% | 我们更好 |
| 组件标识 | ❌ 仅 1 处 | ⚠️ 约 60% | 我们更好 |
| trace_id | ❌ 无 | ❌ 无 | 都需改进 |
| 综合评分 | 55/100 | 75/100 | **我们明显更好** |
...
1. **参数格式统一**: 所有参数使用 `key=%s` 格式
2. **添加关键参数**: 添加与错误相关的关键参数
3. **添加期望值/实际值**: 明确说明期望值和实际值
4. **添加排查方向**: 约 50% 的 ERROR/WARNING 添加了排查方向
5. **添加组件标识**: 约 60% 的日志添加了组件标识
